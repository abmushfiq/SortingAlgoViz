...
public class BubleSort   extends JPanel {
	...
	sorter = new SwingWorker<>() {
		...
		public Void doInBackground() throws InterruptedException {
			// here is buble sort algorithms;
			for(int i = bar_height.length-1; i>0; i--) {
				swaped = false;
				for(int j = 0 ; j< i ; j++ ) {
				...
				// if you did not swap for a particular value of i, its means the arry is sorted hence stop the loop
				if(!swaped) {
					break;
				}
			}
